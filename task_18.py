...
A_integer = input("Введите элементы списка через пробел: ").split()
A_num = list(map(int, A_integer))
if len(A_num) != N or N == 0:
    ...
    for i in range(1, N):
        ...
        if count < min:
            min = count
            index = i
    print(f'Число {A_num[index]} в списке A наиболее близко по величине\
           к числу {X},\ их разница составляет {abs(X - A_num[index])}')